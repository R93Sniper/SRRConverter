...
def ProcessMaterials(SourceFile: SrmFile, WorkingNode: fbx.FbxNode, texture_format):
    texture_converter = TextureConverter(fmt=texture_format)
    for Material in SourceFile.material_palette.materials:
        mat = fbx.FbxSurfacePhong.Create(SceneManager, Material.name)
        for texture in Material.get_texture_suffixes():
            try:
                ...
                fbx_tex = fbx.FbxFileTexture.Create(SceneManager, texture)
...
def ProcessTriangleInfo(SourceFile: SrmFile, WorkingMesh: fbx.FbxMesh, WorkingNode: fbx.FbxNode, WorkingLayer: fbx.FbxLayer):
    ...
    ProcessMaterials(SourceFile, WorkingNode, "PNG")
    ...
    for i, tri in enumerate(SourceFile.display_buffer.indices):
        WorkingMesh.BeginPolygon(SourceFile.display_buffer.vertices[tri[0]].texture_index - 1)
    ...
    # for MatIndex in range(len(SourceFile.material_palette.materials)):
